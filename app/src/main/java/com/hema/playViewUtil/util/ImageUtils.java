...
public class ImageUtils {
	...
	public static Bitmap rotate(Bitmap b, int degrees) {
		if (degrees != 0 && b != null) {
			...
			try {
				...
				LogUtils.e("OutOfMemoryError",ex.toString());
			} catch (Exception ex) {
	...
	public static Bitmap decodeUriAsBitmap(Context ctx, Uri uri) {
		...
		try {
			...
			LogUtils.i("orgi","orgi:" + options.outWidth + "x" + options.outHeight);
			int be = (int) (options.outHeight / (float) 350);
			if (be <= 0)
				be = 1;
			options.inSampleSize = be;// calculateInSampleSize(options,
										// reqWidth, reqHeight);
			LogUtils.i("inSampleSize","inSampleSize:" + options.inSampleSize);
			options.inJustDecodeBounds = false;
